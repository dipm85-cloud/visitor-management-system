...
The native Operations Hub Visitors workspace provides capability-aware visitor summaries, shortcuts and the native Planned Visits workflow. Planned Visits reuses the existing records, field rules, validation conventions, audit events, capabilities and RLS enforcement for list, filter, create and permitted edit actions. The existing Visitor Management Solution remains available as the clearly labelled temporary `Legacy VMS` application for walk-ins, staff sign-in/sign-out, history, reports, configuration, agreements, GDPR, exports and printing until each workflow is migrated and verified. Public kiosk routing continues to open Legacy VMS directly.
