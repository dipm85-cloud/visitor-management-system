...
The native Operations Hub Visitors workspace provides capability-aware visitor summaries, shortcuts, Planned Visits management and authenticated staff Walk-in creation. Planned Visits and Walk-ins reuse the existing records, field rules, validation conventions, privacy acknowledgement, audit and notification helpers, capabilities and RLS enforcement. Walk-in creation preserves the established immediate sign-in behavior without introducing a native sign-in/sign-out management surface. The existing Visitor Management Solution remains available as the clearly labelled temporary `Legacy VMS` application for sign-in/sign-out management, history, reports, configuration, agreements, GDPR, exports and printing until each workflow is migrated and verified. Public kiosk routing continues to open Legacy VMS directly.
