...
The native Operations Hub Visitors workspace provides capability-aware visitor summaries, Planned Visits management, authenticated staff Walk-in creation, planned visitor sign-in, an active on-site list, visitor details and staff sign-out. These workflows reuse the existing records, field rules, validation conventions, privacy and compliance checks, audit and notification/email helpers, capabilities and RLS enforcement. The existing Visitor Management Solution remains available as the clearly labelled temporary `Legacy VMS` application for history, reports, configuration, agreements, GDPR, exports, printing and public kiosk workflows until each area is migrated and verified. Public kiosk routing continues to open Legacy VMS directly.
